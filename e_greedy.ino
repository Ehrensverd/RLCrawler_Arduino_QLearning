...
int e_greedy(float &e_greed, float e_greed_final, float e_greed_step, float Q[25][4], int state)
{
   int action = 0;
   int n, i = 0;
   ...
   //Pick action: e-Greedy:
      ...
      max_q = Q[state][0];     //Initially guess that the first action (index 0) is has the highest Q-val
      for (i=1; i<4; i++) 
        {
        if (Q[state][i] > max_q) //TODO handle decision between two actions with same Q values
          {
          max_q = Q[state][i];
          action = i;
          }        
        } DEBUG_PRINTLN("e-Greedy: Highest Q-Choice");
      n=0;
      for (i=0; i<4; i++)
        {
        if(Q[state][i] == max_q)
